...
_load(ImlibImage *im, int load_data)
{
    ...
    if (y4m.depth == 10 && y4m.colour_space == Y4M_PARSE_CS_420P10)
    {
        ...
        uint8_t* buf = calloc(((y4m.w * y4m.h * 3) >> 1), sizeof(uint8_t));
        if (!buf)
            return LOAD_OOM;
        ...
        I010ToI420(y4m.y, y4m.y_stride, y4m.u, y4m.u_stride, y4m.v, y4m.v_stride, buf_y, buf_stride_y, buf_u, buf_stride_u, buf_v, buf_stride_v, y4m.w, y4m.h);

        /* convert 8-bit YUV to 8-bit ARGB */
        res = conv(buf_y, buf_stride_y, buf_u, buf_stride_u, buf_v, buf_stride_v,
